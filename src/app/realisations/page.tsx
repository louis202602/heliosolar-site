  ...
  return <>
    ...
    <section className="py-24 sm:py-28"><div className="container-px">
      <div className="grid gap-6 md:grid-cols-2">
        {projects.map((p,i)=><Reveal key={p.location} delay={i*90}><article className="group h-full overflow-hidden rounded-5xl border border-ocean-900/10 bg-white shadow-card transition-all duration-500 ease-premium hover:-translate-y-1.5 hover:shadow-card-hover dark:border-white/10 dark:bg-white/[0.035]">
          <div className="relative aspect-[16/10] overflow-hidden"><img src={p.image} alt={`${p.type} — ${p.location}`} className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.03]" loading="lazy"/><div className="absolute inset-0 bg-gradient-to-t from-ocean-950/75 via-transparent to-transparent"/><div className="absolute inset-x-6 bottom-6"><p className="text-3xl font-black tracking-[-0.04em] text-white">{p.power}</p></div></div>
