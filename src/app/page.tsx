...
const sectors = [
  { title: "Résidentiel", metric: "3 à 36 kWc", href: "/residentiel", text: "Autoconsommation, stockage et pilotage pour réduire durablement votre facture." },
  { title: "Professionnels", metric: "36 à 100 kWc", href: "/professionnels-industriels", text: "Toitures d’activité, tertiaire et ombrières dimensionnées autour de vos usages réels." },
  { title: "Industrie & agriculture", metric: "100 à 500 kWc", href: "/professionnels-industriels", text: "Grandes puissances, contraintes techniques complexes et stratégie énergétique à l’échelle du site." },
];
...
function ArrowIcon() {
  return <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true"><path d="M5 12h14M13 6l6 6-6 6" /></svg>;
}

function ProjectMedia({ src, alt, className = "" }: { src: string; alt: string; className?: string }) {
  ...
  return (
    <>
      <section className="relative isolate overflow-hidden bg-ocean-950 text-white">
        <div className="solar-grid pointer-events-none absolute inset-0 opacity-25" />
        ...
        <div className="relative container-px grid min-h-[720px] items-center gap-14 py-20 lg:grid-cols-[0.9fr_1.1fr] lg:py-24">
          <div className="hero-rise max-w-3xl">
            <div className="mb-7 inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-4 py-2 backdrop-blur"><span className="h-2 w-2 rounded-full bg-solar-300 shadow-[0_0_14px_rgba(255,215,94,0.9)]" /><span className="text-[11px] font-semibold uppercase tracking-[0.22em] text-ocean-100">Ingénierie photovoltaïque • Marseille & région Sud</span></div>
            ...
            <div className="mt-12 grid max-w-2xl grid-cols-2 gap-5 border-t border-white/10 pt-7"><div><p className="text-2xl font-bold">QualiPV 500</p><p className="mt-1 text-xs text-ocean-300">Qualification photovoltaïque</p></div><div><p className="text-2xl font-bold">360°</p><p className="mt-1 text-xs text-ocean-300">Étude, pose et suivi</p></div></div>
          </div>
          <div className="hero-rise [animation-delay:120ms]"><div className="relative mx-auto aspect-[4/3] overflow-hidden rounded-[30px] border border-white/10 bg-ocean-900 shadow-panel"><ProjectMedia src="/media/hero-agri.svg" alt="Grande réalisation photovoltaïque agricole HelioSolar vue par drone" className="absolute inset-0" /><div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-ocean-950/55 via-transparent to-transparent" /><div className="absolute bottom-5 left-5 rounded-full border border-white/15 bg-ocean-950/70 px-4 py-2 text-xs font-bold uppercase tracking-[0.16em] backdrop-blur-lg">Réalisation réelle • vue drone</div></div></div>
        </div>
      </section>
